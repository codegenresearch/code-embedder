...
class ReadmeProcessor:
    def __init__(self, script_path_extractor: ScriptPathExtractor, script_content_reader: ScriptContentReader) -> None:
    ...
    def _extract_scripts(self, readme_content: List[str], readme_path: str) -> Optional[List[ScriptMetadata]]:
        scripts = self._script_path_extractor.extract(readme_content)
        if not scripts:
    ...
    def _read_script_content(self, scripts: List[ScriptMetadata]) -> List[ScriptMetadata]:
        return self._script_content_reader.read_script_content(scripts)
...
class CodeEmbedder:
    def __init__(self, readme_paths: List[str], script_path_extractor: ScriptPathExtractor, script_content_reader: ScriptContentReader) -> None:
        self._readme_paths = readme_paths
        self._readme_processor = ReadmeProcessor(script_path_extractor, script_content_reader)
