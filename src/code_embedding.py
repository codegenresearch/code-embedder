...
class ScriptMetadataExtractor:
    def extract(self, readme_content: list[str], path: str) -> list[ScriptMetadata]:
        script_path_extractor = ScriptPathExtractor()
        scripts = script_path_extractor.extract(readme_content=readme_content)
        if not scripts:
            logger.info(f"No script paths found in README in path {path}. Skipping.")
            return []
        logger.info(
            f"""Found script paths in README in path {path}:\n            {set(script.path for script in scripts)}"""
        )
        return scripts
...
class CodeEmbedder:
    ...
    def _process_readme(self, readme_path: str) -> None:
        ...
        scripts = self._script_metadata_extractor.extract(readme_content=readme_content, path=readme_path)
        if not scripts:
            return
