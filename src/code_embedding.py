...
class ScriptContentHandler:
    def __init__(self, script_metadata: ScriptMetadata) -> None:
        self._script_metadata = script_metadata

    def read_content(self) -> str:
        try:
            with open(self._script_metadata.path) as script_file:
                return script_file.read()
        except FileNotFoundError:
            logger.error(f"Error: {self._script_metadata.path} not found. Skipping.")
            return ""
...
class CodeEmbedder:
    ...
    def _read_script_content(self, scripts: list[ScriptMetadata]) -> list[ScriptMetadata]:
        ...
        for script in scripts:
            script_content_handler = ScriptContentHandler(script_metadata=script)
            script.content = script_content_handler.read_content()
            if script.content:
